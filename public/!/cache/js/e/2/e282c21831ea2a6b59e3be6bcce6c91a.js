...
(function($){$.JSON=function(type,url,data,success_callback,error_callback,options)
{if(typeof(data)=="function")
{options=error_callback;error_callback=success_callback;success_callback=data;}
options=options||{};var config={url:url,data:data,type:type,success:function(r)
{if(!r||!r.response)
{console.error("Empty response from "+url,r,data);error_callback&&error_callback(null);return;}
switch(r.response)
{case"ok":success_callback&&success_callback(r.message);break;case"error":error_callback&&error_callback(r.message);break;case"auth":window.top.location.href='/logout';break;case"refresh":window.top.location.href='/';break;default:console.error("Unknown response ["+r.response+"] in "+url,data);error_callback&&error_callback(null);}}};return this.ajax($.extend(config,options)).fail(function(){error_callback&&error_callback(null)});};$.notify={notice:function(text)
{return $.pnotify({history:false,text:text});},info:function(text)
{return $.pnotify({history:false,text:text,type:'info'});},success:function(text)
{return $.pnotify({history:false,text:text,type:'success'});},error:function(text,permanent)
{return $.pnotify({history:false,text:text,type:'error',hide:!permanent,closer:!permanent,sticker:!permanent});}};$.alert=function(message,callback)
{bootbox.alert(message,callback);};$.confirm=function(message,ok_callback,cancel_callback)
{bootbox.confirm(message,function(result)
{if(result)
{ok_callback();}
else
{cancel_callback&&cancel_callback();}});};$.downloadFile=function(url,data,load_callback)
{if(typeof(data)=="function")
{load_callback=data;}
var iframe_name="download-iframe-"+(new Date()).getTime();console.log("iframe name "+iframe_name);var iframe=$('<iframe>').attr("name",iframe_name).appendTo("body").hide();iframe.load(function()
{load_callback&&load_callback();iframe.remove();});if(data)
{var form=$("<form>").attr("action",url).attr("method","post").attr("target",iframe_name);for(var key in data)
{$("<input type='hidden' />").appendTo(form).attr("name",key).attr("value",data[key]);}
form.appendTo("body").submit().remove();}
else
{iframe.attr("src",url);}};})(jQuery);!function($){"use strict";$(function(){$.support.transition=(function(){var transitionEnd=(function(){var el=document.createElement('bootstrap'),transEndEventNames={'WebkitTransition':'webkitTransitionEnd','MozTransition':'transitionend','OTransition':'oTransitionEnd otransitionend','transition':'transitionend'},name
