...
class Kohana_ORM extends Model implements Serializable
{
    ...
    public function clear()
    {
        ...
        $this->_object = $this->_changed = $this->_related = $this->_related_has_many = $this->_original_values = [];
    ...
    public function with(string $target_path, bool $select = null)
    {
        ...
        if (isset($parent->_belongs_to[$target_alias])) {
            ...
            if (isset($parent->_has_one[$target_alias])) {
    ...
    public function find_all()
    {
        ...
        foreach ($this->_load_with as $alias) {
            ...
            if (isset($this->_has_many[$alias]['through'])) {
                ...
                $fkFullCol = $hasManyModel->object_name().'.'.$fkCol;

                /** @var self[] $hasManyData */
                $hasManyData = $hasManyModel
                    ->where($fkFullCol, 'IN', $ids)
                    ->find_all()
                    ->as_array();

                // Detect source <=> target relations
                foreach ($hasManyData as $hasManyItem) {
                    $rowFk = $hasManyItem->get($fkCol);

                    // Many-to-many needs array init
                    $relData[$rowFk] = $relData[$rowFk] ?? [];

                    $relData[$rowFk][] = $hasManyItem;
                }
            }
    ...
    public function count_relations($alias, $far_keys = null)
    {
        ...
        if (!$pk) {
            throw new DomainException('Missing primary key value for '.$this->table_name());
        }

        if ($far_keys === null) {
            return (int)DB::select([DB::expr('COUNT(*)'), 'records_found'])
                ->from($this->_has_many[$alias]['through'])
                ->where($this->_has_many[$alias]['foreign_key'], '=', $pk)
    ...
    public function count_all()
    {
        ...
        $query = $this->_db_builder->from([$this->_table_name, $this->_object_name])
